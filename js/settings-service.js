(() => {
  ...
  const DEFAULTS = {
    ...
    batchSettings: { mode: "resize", resizeMethod: "stretch", width: 512, height: 512, x: 0.5, y: 0.5 }
  };
  ...
  function normalizeBatch(raw) { const d = DEFAULTS.batchSettings; return { mode: pick(raw.mode, ["resize", "crop"], d.mode), resizeMethod: pick(raw.resizeMethod, ["stretch", "cover", "contain"], d.resizeMethod), width: size(raw.width, d.width), height: size(raw.height, d.height), x: unit(raw.x, d.x), y: unit(raw.y, d.y) }; }
