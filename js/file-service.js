...
function conversionBaseName(fileName) { return sanitizeBaseName(fileName).replace(/(?:_crop)?_\d+x\d+$/, "") || "image"; }
function makeResizedFileName(originalFileName, width, height, format) { return `${conversionBaseName(originalFileName)}_${width}x${height}.${EXT[format] || "png"}`; }
function makeCroppedFileName(originalFileName, width, height, format) { return `${conversionBaseName(originalFileName)}_crop_${width}x${height}.${EXT[format] || "png"}`; }
